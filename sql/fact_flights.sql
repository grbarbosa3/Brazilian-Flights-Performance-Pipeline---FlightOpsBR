...
WITH enriched AS (
    SELECT
        ...
        ROUND(
            (EXTRACT(EPOCH FROM (f.real_departure_ts - f.est_departure_ts)) / 60)::numeric,
            2
        ...
        ROUND(
            (EXTRACT(EPOCH FROM (f.real_arrival_ts - f.est_arrival_ts)) / 60)::numeric,
            2
        ...
        *,
        ROW_NUMBER() OVER (
            ...
            ORDER BY
                ...
                real_departure_ts NULLS LAST,
                real_arrival_ts NULLS LAST
        ) AS rn
    FROM enriched
)
SELECT
